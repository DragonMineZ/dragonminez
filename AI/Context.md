...
- `release-prepare.yml` — runs only on pushes to `main` that touch `gradle.properties` (a version bump) plus manual dispatch. Validates stable release candidates, builds the jar, writes manifest artifacts (via `scripts/release/prepare_release.py`), and posts the candidate payload to the Discord bot over SSH at `http://127.0.0.1:8088/dmz-release` (secret `DMZ_RELEASE_BOT_WEBHOOK_SECRET`). It goes through the VPS rather than `release-bot.dragonminez.com` because the public hostname sits behind a Cloudflare managed challenge that answers curl with a 403.
- `release-publish.yml` — triggered by Discord-bot approval through `repository_dispatch` (`dragonminez_release_approved`) or manual dispatch with the same payload fields. Verifies the approved commit is still `origin/main` HEAD, rebuilds, publishes to Modrinth and CurseForge, updates `update.json` (via `scripts/release/update_forge_update_json.py`), and commits it back to `main`.
