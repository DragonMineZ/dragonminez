...
- Upgrade results accumulate in `QuestUpdateReport` (cleared each `loadAll`): a summary is logged, written to `<world>/dragonminez/quest_update_report.txt`, and pushed to operators (`hasPermissions(2)`) in chat on login via `QuestEvents.onPlayerLogin`. The whole feature is gated by the `autoUpdateQuests` server-config flag (default true; false = legacy generate-only-if-missing).
