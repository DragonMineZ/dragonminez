...
- One full-resolution mask per frame, premultiplied by coverage. Shaders with a `location = 1` output draw through `beginCapture`/`endCapture` (bound scene colour + mask + scene depth in one framebuffer). Everything else is queued with `AuraRenderer.captureBloom`/`captureAuraBloom`/`captureLightningBloom` (ki attacks are their `KiRenderTask`s) and redrawn in its `bloomMode` through `beginRedraw`/`endRedraw` (mask + the main target's depth-stencil, so walls and the player's stencil mask still hide the glow). Under Iris only the redraw path runs.
