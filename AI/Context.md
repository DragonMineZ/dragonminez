...
- Shared by every style: the side meters (`HudSideMeters`; `KiReserveHUD` = Frost Demon reserve tinted with the player's ki/aura colour, max = `maxEnergy * reserveMaxRatio`; `RageMeterHUD` = Mutant rage, dark green). Also shared: `PartyHUD`.
- Party HUD data: `PartyPackets.sendHudSync` (from `TickHandler`, same 10-tick cadence as the stats sync) sends each party member the nearest `HUD_MAX_MEMBERS` (3) online allies with name/HP/Ki/distance as `HudSyncS2C` -> `PartyHudCache` (entries go stale after 2.5 s, cleared on disconnect). When the ally entity is loaded client-side the row reads live entity health and the synced capability instead, so bars move every frame; otherwise it shows the packet values, the distance, and the skin face.
