...
- **Quests/sagas/sidequests:** loaded from world-save JSON under `<world>/dragonminez/{sagas,quests,sidequests}` by `QuestRegistry.loadAll` at server start. Files carry a `"defaultsVersion"` stamp and are **version-upgraded in place** by `QuestUpgrader` — a three-way merge (user file vs shipped baseline in `resources/data/dragonminez/previousQuests/` vs new code default) that adopts new defaults for untouched fields, keeps player edits, and reports conflicts (see `AI/Context.md` for the merge rule and the required per-release baseline snapshot). Gated by the `autoUpdateQuests` server config (default on). Preserve saga filename ordering (`01_...json`) and translation-key titles.
