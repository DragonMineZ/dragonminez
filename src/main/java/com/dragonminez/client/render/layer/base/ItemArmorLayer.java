 ...
 */
public class ItemArmorLayer<T extends DMZAnimatable> extends GeoRenderLayer<T> {
    ...
    public ItemArmorLayer(GeoRenderer<T> geoRenderer) {
        super(geoRenderer);
    }
    ...
    protected EquipmentSlot getEquipmentSlotForBone(GeoBone bone, ItemStack stack, T animatable) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            if (slot.getType() == EquipmentSlot.Type.ARMOR) {
                if (stack == player.getItemBySlot(slot))
                    return slot;
            }
        }
        ...
        if (armorStack.getItem() instanceof BlockItem blockItem && blockItem.getBlock() instanceof AbstractSkullBlock skullBlock) {
            ...
            if (!modelPart.cubes.isEmpty()) {
                ...
                if (model instanceof GeoArmorRenderer<?> geoArmorRenderer) {
                    ...
                    geoArmorRenderer.prepForRender(player, armorStack, slot, model);
     ...
     */
    protected <I extends Item & GeoItem> void renderVanillaArmorPiece(PoseStack poseStack, T animatable, GeoBone bone,
                                                                      EquipmentSlot slot, ItemStack armorStack,
                                                                      ModelPart modelPart,
        ...
        ResourceLocation texture = getVanillaArmorResource(player, armorStack, slot, "");
        ...
        if (armorStack.getItem() instanceof DyeableArmorItem dyable) {
            ...
            texture = getVanillaArmorResource(player, armorStack, slot, "overlay");
     ...
     */
    protected VertexConsumer getArmorBuffer(MultiBufferSource bufferSource, @Nullable RenderType renderType,
                                            @Nullable ResourceLocation texturePath, boolean enchanted) {
        if (renderType == null)
    ...
    protected HumanoidModel<?> getModelForItem(GeoBone bone, EquipmentSlot slot, ItemStack stack, T animatable) {
        ...
        return IClientItemExtensions.of(stack).getHumanoidArmorModel(player, stack, slot, defaultModel);
    }
