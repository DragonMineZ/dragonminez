	...
	public void tick(BlockState state, ServerLevel world, BlockPos pos, RandomSource random) {
		if (!world.isClientSide) {
			...
			if (blockAbove.getLightBlock(world, pos.above()) > 2 && world.getMaxLocalRawBrightness(pos.above()) < 4) {
				world.setBlockAndUpdate(pos, MainBlocks.ROCKY_DIRT.get().defaultBlockState());
			} else {
				if (world.getMaxLocalRawBrightness(pos.above()) >= 9) {
					for (int i = 0; i < 4; ++i) {
						...
						if (targetState.is(MainBlocks.ROCKY_DIRT.get()) &&
								aboveTargetState.isAir() &&
								world.getMaxLocalRawBrightness(targetPos.above()) >= 4) {
							world.setBlockAndUpdate(targetPos, MainBlocks.SACRED_PLANET_GRASS_BLOCK.get().defaultBlockState());
						}
					}
				}
			}
