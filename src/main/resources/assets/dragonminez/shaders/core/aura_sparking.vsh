...
void main() {
    ...
    vec3 noiseCoord = vec3(Position.x * 3.5, Position.y * 2.5 - (time * speed), Position.z * 3.5);
    float n = abs(snoise(noiseCoord));
    n = 1.0 - n;
    n = pow(n, 2.5);

    float macroNoise = snoise(vec3(Position.x * 1.5, Position.y * 1.0 - (time * 6.0), Position.z * 1.5));
    float displacement = (n * 0.75 + macroNoise * 0.35);

    float falloff = smoothstep(-1.8, 0.0, Position.y) * (1.0 - smoothstep(0.0, 2.0, Position.y));

    float taper = 1.0 - (smoothstep(0.2, 1.0, Position.y) * 0.8);
    pos.xz *= taper;
